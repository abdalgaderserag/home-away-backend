...
<body class="flex items-center justify-center min-h-screen p-4">
    <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-2xl">
        ...
        <div id="notifications-container" class="space-y-4">
            ...
            <p class="text-center text-gray-500" id="no-notifications-message">Click "Fetch Notifications" to see your alerts.</p>
        </div>
    </div>
                ...
                if (response.ok) {
                    if (data.notifications && data.notifications.length > 0) {
                        data.notifications.forEach(notification => {
                            ...
                            notificationElement.className = 'bg-gray-50 p-4 rounded-lg shadow-sm border border-gray-200';
                            notificationElement.innerHTML = `
                                <h3 class="font-semibold text-lg text-gray-800">${notification.title || 'Notification'}</h3>
                                <p class="text-gray-600 mt-1">${notification.message || 'No message provided.'}</p>
                                ${notification.timestamp ? `<p class="text-gray-400 text-sm mt-2">${new Date(notification.timestamp).toLocaleString()}</p>` : ''}
                            `;
                    ...
                    const errorMessage = data.message || (data.errors ? Object.values(data.errors).flat().join('; ') : 'Failed to fetch notifications.');
                    showMessage(errorMessage, 'error');
                }
            } catch (error) {
                console.error('Error fetching notifications:', error);
                showMessage('An unexpected error occurred while fetching notifications. Please try again.', 'error');
            }
        });
